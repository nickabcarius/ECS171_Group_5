...
class LinearRegression:
    ...
    def fit(self, x, y, order):
        ...
        self.X = np.c_[np.ones(
            len(x))]  # initialize X as a matrix having columns of ones. This corresponds to polynomial of order 0
        while True:
...
def StateLinearRegression(ax1, state, input_col, start_day, end_day):
    for key in dict:
...
def PlotBar(ax1, state, input_col, start_day=0, end_day=400):
    for key in dict:
        if dict[key]['subregion1_name'][0] == state:
            df_state = dict[key]
            break
    ...
    ax1.bar(list(x), list(y), color='tab:blue')
    ax1.set_xticks(np.arange(0, max(x) + 1, 20))
    ax1.set_xlabel('Days', size=10)
    ax1.set_ylabel(graph_labels[input_col], size=10)
    ax1.set_title("{0}'s Bar plot of Days vs {1}".format(state, graph_labels[input_col]), size=10)
...
def BarAndRegression(state, input_col, start_day, end_day):
    PlotBar(ax1, state, input_col, start_day, end_day)
    for key in dict:
        if dict[key]['subregion1_name'][0] == state:
            df_state = dict[key]
            break
...
graph_labels = {
  ...
  "new_deceased": "New Deceased Cases",
  "new_recovered": "New Recovered Cases",
  "new_tested": "New Tested Cases",
  "total_confirmed": "Total Confirmed Cases",
  "total_deceased": "Total Deceased Cases",
  "total_recovered": "Total Recovered Cases",
  "total_tested": "Total Tested Cases",
  "new_hospitalized": "New Hospitalized Cases",
  "total_hospitalized": "Total Hospitalized Cases",
  "current_hospitalized": "Current Hospitalized Cases",
  "new_intensive_care": "New Intensive Care Cases",
  "total_intensive_care": "Total Intensive Care Cases",
  "current_intensive_care": "Current Intensive Care Cases",
  "new_ventilator": "New Ventilator Cases",
  "total_ventilator": "Total Ventilator Cases",
  "current_ventilator": "Current Ventilator Cases",
  "population": "Population",
  "population_male": "Male Population",
  "population_female": "Female Population",
  "population_density": "Population Density",
  "population_age_00_09": "Population Age (0-9)",
  "population_age_10_19": "Population Age (10-19)",
  "population_age_20_29": "Population Age (20-29)",
  "population_age_30_39": "Population Age (30-39)",
  "population_age_40_49": "Population Age (40-49)",
  "population_age_50_59": "Population Age (50-59)",
  "population_age_60_69": "Population Age (60-69)",
  "population_age_70_79": "Population Age (70-79)",
  "population_age_80_89": "Population Age (80-89)",
  "population_age_90_99": "Population Age (90-99)",
  "population_age_80_and_older": "Population Age (80 and older)",
  "ndays": "Days"
}
...
class TimeSeriesForecast:
    ...
    def plot_data(self):
        ...
        ax1.fill_between(self.forecast.index, self.forecast['yhat_upper'], self.forecast['yhat_lower'], alpha=0.5,
                         color='darkgrey')
...
class ARIMAForecast:
    ...
    def buildForecast(self):
        ...
        self.prediction = \
        model_fit.predict(start=model_fit.fittedvalues.index[0], end=date.today(), typ='levels').values[0]
        self.fc = model_fit.predict(start=model_fit.fittedvalues.index[0], end=model_fit.fittedvalues.index[-1],
                                    typ='levels').values
...
def graph(state, day, graph_type):
    plt.clf()
    if (state == ""):
        state = "California"
    day = int(day)
    ...
    print("graphing")
    table = Stateinfo(state, day)
    stats = []
    i = 0
    for col in table:
        i += 0.03
        node = [graph_labels[col], table[col][state]]
        stats.append(node)
    print(i)
    print(stats)

    fig, (ax1, ax2) = plt.subplots(2, figsize=(10, 8), gridspec_kw={'height_ratios': [8, 1]})

    fig.subplots_adjust(bottom=i, hspace=1)

    ax2.table(cellText=stats,
              cellLoc='center',
              loc='bottom')
    ax2.spines['bottom'].set_visible(0)
    ax2.spines['top'].set_visible(0)
    ax2.spines['left'].set_visible(0)
    ax2.spines['right'].set_visible(0)
    for t in ax2.xaxis.get_ticklines(): t.set_visible(0)
    for t in ax2.yaxis.get_ticklines(): t.set_visible(0)
    ax2.set_xticks([])
    ax2.set_facecolor(color='w')
    ax2.set_yticks([])
    # ax2.tight_layout()

    # Works
    PlotBar(ax1, state, graph_type, 0, day)

    # Takes way to long to run
    # StateLinearRegression(state, 'total_confirmed', 0, day)
    # CallTimeSeriesForecast('New York', 'total_confirmed')
    # Takes way to long to run
    # BarAndRegression(state, 'current_hospitalized', 0, day)

    # Needed for website graphing
...
#     print("graphing")
#
#     StateLinearRegression(state, graph_type, 0, day)
#
#     # Needed for website graphing
#     for filename in os.listdir('static/'):
#         if filename.startswith('plot'):
...
def graph2(state, graph_type):
    ...
    CallTimeSeriesForecast(state, graph_type)

    # Takes way to long to run
    #BarAndRegression(state, graph_type, 0, day)

    # Needed for website graphing
    for filename in os.listdir('static/'):
        if filename.startswith('plot2'):
            os.remove('static/' + filename)
...
def graph3(state, graph_type):
    ...
    graph_type = graph_type
    print("graphing")
    CallARIMAForecast(state, graph_type)

    # Needed for website graphing
    for filename in os.listdir('static/'):
        if filename.startswith('plot3'):
            os.remove('static/' + filename)
